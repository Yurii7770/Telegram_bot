...
class Database:
    ...
    def save_pending_post(self, item_id: str, author: str, title: str, post_text: str,
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute(
                ...
                (tweet_id, author, title, post_text, twitter_post, has_media, media_urls, suggested_tags, sniper_reply, target_platform, ai_opinion, source_url, status)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'PENDING')
                """,
                (
    ...
    def get_pending_post(self, db_id: int) -> Optional[dict]:
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                SELECT id, tweet_id, author, title, post_text, twitter_post, has_media, media_urls, sniper_reply, target_platform, ai_opinion, source_url, status
